...
def noise(time_series):
    ...
    if fluc_extension != 1:
        fluc_place = np.random.randint(0, time_series.shape[0] - (time_series.shape[0] * fluc_extension))
    else:
        fluc_place = 0
    ...
    for i in range(int(fluc_extension * time_series.shape[0])):
        ...
        time_series_z[fluc_place + i] = time_series_z[fluc_place + i] + np.random.normal(0, fluc_variation)
