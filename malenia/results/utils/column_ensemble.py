...
def ensemble_columns(results):
    ...
    for method_pretty, method_real in results.methods.items():
        ...
        for dataset, seed, results_path in results._method_results_info_generator(
            method_real, "test"
        ):
            ...
            probas_list = [
                np.fromstring(string[1:-1], sep=" ").astype(np.float64)
                for string in df["y_proba"].values
            ]
    ...
    for index, predictions in manager_ensembled.items():
        ...
        if "_" in Method:
            ...
            method_dataset_results_path = os.path.join(
                results.results_path, method_general, method_specific, Dataset
            )
        else:
            method_dataset_results_path = os.path.join(results.results_path, Method, Dataset)

        existing_preds = pd.read_csv(
            os.path.join(method_dataset_results_path + "_dim_0", "seed_" + Seed + "_test.csv")
        )
