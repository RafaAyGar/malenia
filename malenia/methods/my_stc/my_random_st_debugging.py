...
class MyRandomShapeletTransformDebug(BaseCollectionTransformer):
    # _tags = {
    ...
    def __init__(
        ...
        super(MyRandomShapeletTransformDebug, self).__init__()
    ...
    def _fit(self, X, y=None):
        ...
        shapelets = list([list([(-1.0, -1, -1, -1, -1, -1)]) for _ in range(self.n_classes_)])
        ...
        if time_limit > 0:
            ...
            while n_shapelets_extracted < self.n_shapelet_samples:
                n_shapelets_to_extract = (
                    self._batch_size
                    if n_shapelets_extracted + self._batch_size <= self.n_shapelet_samples
        ...
        ##
        #
        self.shapelets.sort(reverse=True, key=lambda s: (abs(s[0]), -s[1], s[2], s[3], s[4]))
        ...
        for s in self.shapelets:
            ...
            self._sorted_indicies.append(
                np.array(sorted(range(s[1]), reverse=True, key=lambda j, sabs=sabs: sabs[j]))
            )
    ...
    def _transform(self, X, y=None):
        ...
        for i, series in enumerate(X):
            dists = Parallel(n_jobs=self._n_jobs, backend=self.parallel_backend, prefer="threads")(
    ...
    @staticmethod
    # @njit(fastmath=True, cache=True)
    def _merge_shapelets(shapelet_heap, candidate_shapelets, max_shapelets_per_class, cls_idx):
...
#         )
#         numpy_corrcoef = np.corrcoef(
#             orderline[:, 0], np.power(np.abs(orderline[:, 1] - shp_class), l_norm)
#         )
#         numpy_corrcoef = 0.0 if np.isnan(numpy_corrcoef[0, 1]) else numpy_corrcoef[0, 1]
#         assert math.isclose(r_value, numpy_corrcoef)
#         # if r_value > 0.0:
#         #     print("R:", r_value)
#         #     print("Numpy corrcoef:", numpy_corrcoef)
