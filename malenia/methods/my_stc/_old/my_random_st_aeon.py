...
class RandomShapeletTransform(BaseCollectionTransformer):
    ...
    def _fit(self, X, y=None):
        ...
        shapelets = List([List([(-1.0, -1, -1, -1, -1, -1)]) for _ in range(self.n_classes)])
        ...
        if time_limit > 0:
            ...
            while n_shapelets_extracted < self._n_shapelet_samples:
                n_shapelets_to_extract = (
                    self._batch_size
                    if n_shapelets_extracted + self._batch_size <= self._n_shapelet_samples
    ...
    def _extract_random_shapelet(self, X, y, i, shapelets, max_shapelets_per_class):
        ...
        rs = None if self.random_state is None else (rs * 37 * (i + 1)) % np.iinfo(np.int32).max
    ...
    def _find_shapelet_quality(
        ...
        compute_early_quality = (quality_measure == "ig") and (worst_quality > 0)
        ...
        for i, series in enumerate(X):
            ...
            # When using early quality calculation this sort must be inside the for loop
            orderline.sort()

            if compute_early_quality:
        ...
        if quality_measure == "ig":
            quality = _calc_binary_ig(orderline, this_cls_count, other_cls_count)
        elif quality_measure == "ig_plus_correlation":
            quality = _calc_binary_ig(orderline, this_cls_count, other_cls_count) + (
                0.05 * _calc_correlation(orderline, y[inst_idx], l_norm=1)
            )
        elif quality_measure == "correlation":
